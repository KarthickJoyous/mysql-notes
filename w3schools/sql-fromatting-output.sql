# Link : https://www.w3resource.com/sql-exercises/sql-fromatting-output-exercises.php
...
SELECT CONCAT("For ", ord_date, " there are ", COUNT(*), " orders.") FROM orders GROUP by ord_date;
...
4. From the following table, write a SQL query to find all the orders. Sort the result-set in descending order by ord_date.
Return all fields.
...
SELECT * FROM orders ORDER BY ord_no DESC;
...
Sort the result-set by customer_id.
...
SELECT salesman_id, ord_date, MAX(purch_amt) FROM orders GROUP BY 1,2 ORDER BY salesman_id, ord_date;
...
SELECT cust_name, city, grade FROM customer ORDER BY grade DESC; -- Null values last.

SELECT cust_name, city, grade FROM customer ORDER BY grade IS NULL DESC, grade DESC; -- Null values first.
...
SELECT customer_id, COUNT(*), MAX(purch_amt) as total_orders FROM orders GROUP BY 1 ORDER BY 2 DESC;
...
SELECT ord_date, SUM(purch_amt), (SUM(purch_amt)/100) * 15 as commission FROM orders GROUP BY ord_date ORDER BY ord_date;
...
SELECT
    ord_date,
    total_purch_amt,
    (total_purch_amt / 100) * 15 AS commission
FROM OrderTotals
ORDER BY ord_date;
